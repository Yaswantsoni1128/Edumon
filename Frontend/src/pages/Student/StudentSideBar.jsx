...
const StudentSideBar = () => {
  ...
  // If no user is found, return null or a fallback (e.g., login link or error message)
  if (!id) {
    return <div>Please log in to access this page.</div> // You can show an error or redirect to login
  }
        ...
        <nav className='mt-4'>
          {menuItems.map((item) => (
            <Link
              key={item.path}
              to={item.path}
              className={`flex items-center ${location.pathname === item.path ? "bg-sky-800" : "hover:bg-sky-500"} px-4 py-2 rounded-md space-x-4 transition-all duration-200`}>
              {item.icon}
