...
const UpdateTeacherModal = ({ teacher, onClose, onTeacherUpdated }) => {
  const [name, setName] = useState(teacher.name);
  const [subject, setSubject] = useState(teacher.subject);
  const [assignedClass, setAssignedClass] = useState(teacher.assignedClass);
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center z-50">
      <div className="bg-white p-6 rounded-lg shadow-lg w-96">
        ...
        <div className="flex justify-end gap-2">
          ...
          <button className="px-4 py-2 bg-sky-600 text-white rounded" onClick={handleUpdate}>Update</button>
        </div>
      </div>
    </div>
  );
